...
func TestOpenCreatesManagedDirectoriesAndSQLiteFeatures(t *testing.T) {
	...
	if health := core.Health(); !health.Ready || health.SchemaVersion != 4 || health.DataDir != root {
...
func TestMigrationCreatesBackupAndDoesNotRepeat(t *testing.T) {
	...
	if err != nil || len(entries) != 3 {
		t.Fatalf("expected three migration backups, entries=%d err=%v", len(entries), err)
	}
